...
* [4. Control Protocol](#4-control-protocol)
  ...
  * [4.4. JSON-RPC 2.0 Request/Response/Notification messages](#44-json-rpc-20-requestresponsenotification-messages)
    ...
    * [4.4.3. The `remove_service` method](#443-the-remove_service-method)
    * [4.4.4. The `reset_service_table` method](#444-the-reset_service_table-method)
    * [4.4.5. The `scan_network` method](#445-the-scan_network-method)
    * [4.4.6. The `get_status` method](#446-the-get_status-method)
    * [4.4.7. The `get_last_scan_report` method](#447-the-get_last_scan_report-method)
    * [4.4.8. The `update_service_table` notification](#448-the-update_service_table-notification)
...
#### 4.4.3. The `remove_service` method

The `remove_service` method is used by the server to remove a _custom_ service
(i.e. a service added using the `add_service` method defined above) from the
client's service table.

The method has the following parameters:

* `service_id` (integer): 16-bit identifier of the service to be removed.

Upon successful completion, the client MUST respond with a JSON-RPC 2.0
response containing a `result` field with value `null`. No error conditions are
defined for this method.

#### 4.4.4. The `reset_service_table` method

The `reset_service_table` method is used by the server to request the client to
remove all services from its service table with the exception of _custom_ and
_static_ services (i.e. services configured to be always available;
implementation dependent).

The method has the following parameters:

* `full` (boolean, optional): If `true`, the client will remove all services
  that are not marked as _static_. Otherwise, only services that are not marked
  as _custom_ or _static_ will be removed. The default value is `false`.

Upon successful completion, the client MUST respond with a JSON-RPC 2.0
response containing a `result` field with value `null`. No error conditions are
defined for this method.

#### 4.4.5. The `scan_network` method
...
#### 4.4.7. The `get_last_scan_report` method
...
#### 4.4.8. The `update_service_table` notification
